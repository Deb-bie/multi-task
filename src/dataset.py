...
class SynthRADSliceDataset(Dataset):
    ...
    def __init__(
        ...
        # Build flat slice index — expensive for large anatomies on network
        # storage (opens every .mha header). Persist to disk so subsequent
        # runs skip this step entirely.
        self.index: List[Tuple[int, int]] = []
        self._index_cache_path = Path(index_cache_path) if index_cache_path else None
        self._build_index()
    ...
    def _build_index(self) -> None:
        ...
        The index is persisted to ``self._index_cache_path`` (if set) so that
        re-runs skip the expensive per-file header reads on network storage.
        """
        # ── Try loading from disk cache first ───────────────────────────────
        if self._index_cache_path and self._index_cache_path.exists():
            print(f"[dataset] Loading slice index from cache: {self._index_cache_path}",
                  flush=True)
            with open(self._index_cache_path) as f:
                self.index = [tuple(pair) for pair in json.load(f)]
            print(f"[dataset] Index loaded: {len(self.index)} slices", flush=True)
            return

        # ── Build from scratch ───────────────────────────────────────────────
        n = len(self.entries)
        print(f"[dataset] Building slice index for {n} patients "
              f"(split={self.split}) — this may take a few minutes on "
              f"network storage …", flush=True)
        ...
        for entry_idx, entry in enumerate(self.entries):
            ...
            if (entry_idx + 1) % 10 == 0:
                print(f"[dataset]   indexed {entry_idx + 1}/{n} patients, "
                      f"{len(self.index)} slices so far", flush=True)

        print(f"[dataset] Index complete: {len(self.index)} slices", flush=True)

        # ── Persist to disk ──────────────────────────────────────────────────
        if self._index_cache_path:
            self._index_cache_path.parent.mkdir(parents=True, exist_ok=True)
            with open(self._index_cache_path, "w") as f:
                json.dump(self.index, f)
            print(f"[dataset] Index saved to {self._index_cache_path}", flush=True)
