...
def validate(
    ...
    Args:
        ...
        num_classes: Number of segmentation classes including background
                     (default 6).
        compute_seg: If ``False`` (e.g. when ``LAMBDA_SEG=0``), skip
                     segmentation metric computation and return NaN
                     placeholders.  Avoids wasted argmax computation when
                     the segmentation head was never trained.
    ...
    Returns:
        ...
        Segmentation metrics (CT branch — directly GT-supervised via
        TotalSegmentator labels; primary segmentation quality indicator):
        ...
        Segmentation metrics (MRI branch — cross-modal transfer proxy,
        no direct MRI supervision; diagnostic use only):
